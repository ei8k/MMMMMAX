...
─────▀██▀─────
MAX ‿ @deeeeeeeei
تم تطوير وبرمجة السورس من قبل معاذ الحربي
@uuuuuuuuueu ‿ @deeeeeeeei 
#-------------------------------------------------------------------
]]
...
file:close()
return sendDocument(msg.chat_id_,msg.id_,'./inc/All_Groups.html','👨🏽‍✈️قائمه المجموعات بالكامله ✓ \n🗃يحتوي ('..#list..') مجموعه \n🖥افتح الملف في عارض HTML او بالمتصفح',dl_cb,nil)
else 
...
return "أشترك بالقناة أولا["..UserChaneel.."] \nثم ارجـع ارسل تفعيل ."
end
else
return "أشترك بالقناة أولا["..UserChaneel.."] \nثم ارجـع ارسل تفعيل ."
end
...
return sendMsg(msg.chat_id_,msg.id_,'**لا يمكنيي تفعيل البوت في المجموعة ، يجب ان يكون اكثر من*【'..GroupUsers..'】* عضـو ')
else 
GetChatMember(msg.chat_id_,our_id,function(arg,data)
if data.status_.ID == "ChatMemberStatusMember" then
return sendMsg(msg.chat_id_,msg.id_,'**عذرا البوت ليس مشرف في المجموعة\nيجب ترقيتة مشرف لتتمكن من تفعيل البوت ')
else
...
sendMsg(msg.chat_id_,msg.id_,'تـم تـفـعـيـل ألمـجمـوعة \n')
end
GetChannelAdministrators(msg.chat_id_,function(arg,data)
for k,v in pairs(data.members_) do
...
if UserID == our_id then 
return sendMsg(ChatID,MsgID,"**عذرا لا يمكنني رفع نفسي ⁦^_^⁩ \n") 
...
if cmd == "iduser" then
return SendMention(ChatID,UserID,MsgID,"اضغط على الايدي ليتم النسـخ\n\n "..USERNAME.." ~⪼ { "..UserID.." }",37,USERCAR)
end
if cmd == "whois" then
...
if cmd == "iduser" then
return sendMsg(ChatID,MsgID," أضغط على الايـدي ليتم النـسخ\n\n "..UserName.." ~⪼ ( `"..UserID.."` )")
end
...
.."\n️الماركدوان ، "..(redis:get(max..'lock_markdown'..msg.chat_id_) or 'false')
